...
def plumMCMC(dirt, corename, T_mod, num_sup, det_lim, iterations, by, shape1_m, mean_m, shape_acc, mean_acc, fi_mean, fi_acc, As_mean, As_acc, resolution, seeds, thi, burnin, bqkg, Cs, Sdate, CSTdate, dmax):
    seed(int(seeds))
    plomo = "/"+corename+".csv"
    fimean = fi_mean
    shapefi = fi_acc
    ...
    def ln_prior_supp(param):
        ...
        prior = prior - (((1./by)-1.)*log(param[1+Ran]) - log(by) + ((1./by)*(shape1_m-1.))*log(
            param[1+Ran]) + (shape2_m - 1.)*log(1.-param[1+Ran]**(1./by)))  # prior for w	#
        for ms in range(m):
    ...
    for i in range(len(depths[0, :])-1):
        ...
        for k in range(len(logvect)):
            if logvect[k]:
                if i != 0:
                    y1 = estim[:, i-1]+((estim[:, i]-estim[:, i-1]) /
                                        (depths[0, i+1]-depths[0, i]))*(grafdepts[k]-depths[0, i])
                    porc = []
